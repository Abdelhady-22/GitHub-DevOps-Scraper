...
def keep(issue, repo) -> tuple[bool, str]:
...
def discover(client, repo, state):
    slug = repo.slug
    if P.discovery_done(slug, state):
        n = len(P.discovered_ids(slug, state))
        print(f"  [{repo.full_name}:{state}] already done ({n} found)")
        return
    ...
    for items, page in client.list_issues(repo.owner, repo.repo, state, repo.bug_labels, cfg.since):
        ...
        for issue in items:
            ...
            if ok:
                ...
                rejected += 1
                entry = {"number": issue.get("number"), "title": (issue.get("title") or "")[:80], "reason": reason}
        ...
        print(f"  [{repo.full_name}:{state}] page={page:3d} | seen={seen:5d} kept={kept:4d} rejected={rejected:4d} | tokens: {client.tm.status()}")

    (cfg.raw_dir / slug / state / "rejected_summary.json").write_text(json.dumps(rej_log, indent=2))
    P.mark_discovery_done(slug, state, kept)
    print(f"  [{repo.full_name}:{state}] DONE — kept={kept} rejected={rejected} ({rejected/max(seen,1)*100:.0f}% rejection)")
...
def main():
    ...
    print(f"Phase 1 — Discovery  [{datetime.now():%Y-%m-%d %H:%M:%S}]")
    ...
    for repo in repos:
        print(f"\n── {repo.full_name}  labels={repo.bug_labels}")
        for state in states:
            discover(client, repo, state)

    print("\nDone. Next: python phase2_enrich.py")
