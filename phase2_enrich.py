...
def enrich_closed(client, repo):
    ...
    for i, number in enumerate(pending, 1):
        ...
        if ip.exists():
            ...
            if not issue:
                stats["err"] += 1; batch.append(number); _checkpoint(batch, slug, "closed"); batch = []; continue
        ...
        if not pr_nums:
            ...
            stats["no_pr"] += 1; batch.append(number); _checkpoint(batch, slug, "closed"); batch = []; continue
        ...
        if i % 25 == 0:
            print(f"    [{i}/{len(pending)}] ok={stats['ok']} no_pr={stats['no_pr']} err={stats['err']} | {client.tm.status()}")
        if len(batch) >= cfg.checkpoint_every:
            _checkpoint(batch, slug, "closed"); batch = []

    _checkpoint(batch, slug, "closed")
    P.mark_enrichment_done(slug, "closed")
    print(f"  [{repo.full_name}:closed] DONE — ok={stats['ok']} no_pr={stats['no_pr']} err={stats['err']}")


def enrich_open(client, repo):
    slug = repo.slug
    if P.enrichment_done(slug, "open"):
        print(f"  [{repo.full_name}:open] already done"); return

    all_ids = P.discovered_ids(slug, "open")
    done    = P.enriched_ids(slug, "open")
    pending = [n for n in all_ids if n not in done]
    print(f"  [{repo.full_name}:open] {len(pending)} pending")
    ...
    for i, number in enumerate(pending, 1):
        ...
        if not ip.exists():
            issue = client.get_issue(repo.owner, repo.repo, number)
            if issue: ip.write_text(json.dumps(issue))
            else: errors += 1
        batch.append(number)
        if i % 50 == 0:
            print(f"    [{i}/{len(pending)}] errors={errors}")
        if len(batch) >= cfg.checkpoint_every:
            _checkpoint(batch, slug, "open"); batch = []

    _checkpoint(batch, slug, "open")
    P.mark_enrichment_done(slug, "open")
    print(f"  [{repo.full_name}:open] DONE — errors={errors}")


def _checkpoint(batch, slug, state):
    if batch: P.mark_enriched(slug, state, batch)
...
def main():
    ...
    print(f"Phase 2 — Enrichment  [{datetime.now():%Y-%m-%d %H:%M:%S}]")
    ...
    for repo in repos:
        print(f"\n── {repo.full_name}")
        for state in states:
            if state == "closed": enrich_closed(client, repo)
            else: enrich_open(client, repo)

    print("\nDone. Next: python phase3_classify.py")
